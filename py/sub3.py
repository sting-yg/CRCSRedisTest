...
# 目标数据结构（用于数据验证）
EXPECTED_STRUCTURE = {
    "activityType": int,  # activityType 应为 int 类型
    "index": int,  # index 应为 int 类型
    "id": str,  # id 应为 string 类型
    "isAsync": bool,  # isAsync 应为 bool 类型
    "isTaskOverridable": bool,  # isTaskOverridable 应为 bool 类型
    "palletSize": (type(None), int),  # palletSize 可为 None 或 int 类型
    "palletHeight": int,  # palletHeight 应为 int 类型
    "moveParameter": dict,  # moveParameter 应为字典
    "dockingInParameter": dict,  # dockingInParameter 应为字典
    "dockingOutParameter": dict,  # dockingOutParameter 应为字典
    "conveyorParameter": dict,  # conveyorParameter 应为字典
    "standbyParameter": dict,  # standbyParameter 应为字典
    "chargeStartParameter": dict,  # chargeStartParameter 应为字典
    "tableTurnParameter": dict,  # tableTurnParameter 应为字典
    "kivaTurnParameter": dict,  # kivaTurnParameter 应为字典
    "liftParameter": dict,  # liftParameter 应为字典
    "controlPtzParameter": dict,  # controlPtzParameter 应为字典
    "getPictureParameter": dict,  # getPictureParameter 应为字典
    "uploadRequestParameter": dict,  # uploadRequestParameter 应为字典
    "visible": bool  # visible 应为 bool 类型
}
...
def validate_move_data(data):
    ...
    for key, expected_type in EXPECTED_STRUCTURE.items():
        ...
        if isinstance(expected_type, tuple):
            if not isinstance(data[key], expected_type[0]) and data[key] is not None:
    ...
    # 进一步检查 pathList
    if isinstance(data["moveParameter"], dict) and isinstance(data["moveParameter"].get("pathList"), list):
